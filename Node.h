...
{
template<class Key>
class Node : public boost::enable_shared_from_this<Node<Key>>{
public:
	...
	static auto Create(boost::asio::io_service& service, int port, int buffer_size, 
		...
		auto node_ptr = Pointer(new Node(service, key_pickupper, 
				for_upper_func_dict, for_lower_func_dict, os));
		auto core_ptr = P2pCore::Create(service, port, buffer_size, 
				boost::bind(&Node<Key>::CallMatchFuncForUpper, node_ptr, _1, _2, _3),
				boost::bind(&Node<Key>::CallMatchFuncForLower, node_ptr, _1, _2, _3),
				os);
		node_ptr->SetCorePtr(core_ptr);
		return node_ptr;
	}
...
private:
	Node(boost::asio::io_service& service, KeyPickupper key_pickupper, 
