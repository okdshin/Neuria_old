...
public:
	...
	Node(boost::asio::io_service& service, int port, int buffer_size, KeyPickupper key_pickupper, std::ostream& os)
		...
		core_ptr(P2pCore::Create(service, port, buffer_size, 
			boost::bind(&Node::CallMatchFunc, this, &this->for_upper_func_dict, _1, _2, _3), 
			boost::bind(&Node::CallMatchFunc, this, &this->for_lower_func_dict, _1, _2, _3),
			os)){}

	auto RegisterForUpperFunc(const Key& key, P2pCore::OnReceiveFunc func) -> void {
		this->for_upper_func_dict[key] = func;
	}
	
	auto RegisterForLowerFunc(const Key& key, P2pCore::OnReceiveFunc func) -> void {
